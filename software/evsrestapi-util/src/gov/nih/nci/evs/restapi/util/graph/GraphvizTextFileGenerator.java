...
public class GraphvizTextFileGenerator {
	...
	{
		return gov.nih.nci.evs.restapi.util.graph.GraphGenerator.readFile(filename);
	}
    ...
    public void generateGraphvizTextFile(String relationshipFile, String outputfile) {
         ...
         try {
			...
			for (int i=0; i<rel_vec.size(); i++) {
				...
				t = t.trim();
				if (t.length() > 0){
					Vector u = gov.nih.nci.evs.restapi.util.graph.GraphGenerator.parseData(t);
					...
					if (!hset.contains(label_1)) {
						...
						String color = gov.nih.nci.evs.restapi.util.graph.GraphGenerator.generateColor();
	...
	public static void main(String[] args) throws Exception {
        ...
        String format = "png";
        gov.nih.nci.evs.restapi.util.graph.GraphGenerator gg = new gov.nih.nci.evs.restapi.util.graph.GraphGenerator();
