...
public class MetaSourceElasticLoadServiceImpl extends BaseLoaderService {
        ...
        try (final PushBackReader mrconso = readers.getReader(RrfReaders.Keys.MRCONSO);
            ...
            while ((line = mrrel.readLine()) != null) {
                ...
                if (fields[10].toLowerCase().equals(terminology.getTerminology())
                        && fields[3].equals("PAR") && !srcAuis.contains(fields[5])
        ...
        try (final PushBackReader mrconso = readers.getReader(RrfReaders.Keys.MRCONSO);
            ...
            while ((line = mrconso.readLine()) != null) {
                ...
                if (prevCui != null && (cui + "|").compareTo(prevCui + "|") < 0) {
                    throw new Exception(
                            "File is unexpectedly out of order = " + prevCui + ", " + cui);
                }
                ...
                if (sab.toLowerCase().equals(terminology.getTerminology())) {
                    ...
                    if (!codeConceptMap.containsKey(code)) {
                        ...
                        if (concept.getName() == null) {
                            throw new Exception("Unable to find preferred name, likely TTY issue ("
                                    + terminology.getTerminology() + ".json) = "
                                    + terminology.getTerminology().toUpperCase() + ", " + code);
                        }
            ...
            if (codes.size() > 0) {
                for (final String code : codes) {
                    ...
                    if (codeCuisMap.get(code).isEmpty()) {
                        ...
                        if (sourceConcepts++ % 5000 == 0) {
                            logger.info(
                                    "    " + terminology.getTerminology() + " = " + sourceConcepts);
                        }

                    } else {
                        // If not, then something went wrong, should not be possible
                        throw new Exception("Unexpected additional CUIs for code not covered = "
                                + codeCuisMap.get(code));
                    }
                }
            }
            // force index
    ...
    public void handleRelationships(final HierarchyUtils hierarchy, final Terminology terminology,
        final Set<String> codes, final PushBackReader mrrel, final String prevCui)
        throws Exception {
    ...
    private void handleConcept(Concept concept, List<Concept> batch, boolean flag, String indexName)
        ...
        if (conceptSize > 10000000) {
            logger.info(
                    "    BIG concept = " + concept.getCode() + " = " + concept.toString().length());
        }
            ...
            try {
                TerminologyMetadata metadata =
                        new ObjectMapper().readValue(
                                IOUtils.toString(term.getClass().getClassLoader()
                ...
                try {
                    String welcomeText = new ObjectMapper().readValue(IOUtils.toString(
                            term.getClass().getClassLoader().getResourceAsStream(resource),
                            "UTF-8"), String.class);
