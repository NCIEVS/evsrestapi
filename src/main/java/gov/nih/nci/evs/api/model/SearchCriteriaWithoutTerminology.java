...
public class SearchCriteriaWithoutTerminology extends BaseModel {
     ...
     * @throws Exception the exception
     */
    public void validate(final Terminology terminology, final MetadataService metadataService, boolean export)
            throws Exception {
        // if (getTerm() == null) {
        // throw new ResponseStatusException(HttpStatus.BAD_REQUEST,
        ...
        if (getDefinitionSource().size() > 0) {
            final Set<String> definitionSources = metadataService.getDefinitionSources(terminology.getTerminology())
                    .stream()
                    .map(c -> c.getCode()).collect(Collectors.toSet());
