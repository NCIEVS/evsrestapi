...
public class StardogElasticLoadServiceImpl extends BaseLoaderService {
	...
	private void loadConceptsRealTime(List<Concept> allConcepts, Terminology terminology, HierarchyUtils hierarchy)
		...
		while (start < total) {
			...
			List<Concept> concepts = sparqlQueryManagerService.getConcepts(allConcepts.subList(start, end), terminology,
					hierarchy);
			...
			while (indexStart < indexTotal) {
				...
				futures.add(executor.submit(new ConceptLoadTask(concepts.subList(indexStart, indexEnd),
						start + indexStart, start + indexEnd, terminology.getIndexName(), latch, taskSize.intValue())));
	...
	private class ConceptLoadTask implements Callable<Void> {
		...
		public Void call() throws Exception {
			try {
				...
				taskLogger.info("    finish loading concepts: {} to {} ({}% complete)", startIndex + 1, endIndex,
						progress);
			} catch (Throwable e) {
	...
	@Override
	public Terminology getTerminology(ApplicationContext app, ElasticLoadConfig config) throws Exception {
		TerminologyUtils termUtils = app.getBean(TerminologyUtils.class);
		return termUtils.getTerminology(config.getTerminology(), false);
	}
