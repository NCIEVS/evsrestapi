...
public class MetadataServiceImpl implements MetadataService {
   ...
   */
  @Override
  public Map<String, List<StatisticsEntry>> getSourceStats(String terminology, String source) throws Exception {
    final Terminology term = termUtils.getTerminology(terminology, true);
  ...
  public List<ConceptMinimal> getSynonymSources(String terminology) throws Exception {
    ...
    final Set<String> sources =
        esQueryService.getSynonymSources(term).stream().map(c -> c.getCode()).collect(Collectors.toSet());
  ...
  public List<Concept> getSubsets(String terminology, Optional<String> include, Optional<String> list)
    ...
    if (!list.isPresent()) {
      ...
      subsets.stream().flatMap(Concept::streamSelfAndChildren).peek(c -> ConceptUtils.applyInclude(c, ip))
