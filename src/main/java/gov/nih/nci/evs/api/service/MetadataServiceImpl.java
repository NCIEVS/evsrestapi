...
public class MetadataServiceImpl implements MetadataService {
    ...
    // #include.orElse(''), #terminology}",
    // condition = "#list.orElse('').isEmpty()")
    public List<Concept> getRoles(String terminology, Optional<String> include,
        Optional<String> list) throws Exception {
    ...
    /* see superclass */
    @Override
    public Optional<Concept> getSynonymType(String terminology, String code,
        Optional<String> include) throws Exception {
    ...
    public List<Concept> getSubsets(String terminology, Optional<String> include,
        ...
        if (!list.isPresent()) {
            subsets.stream().flatMap(Concept::streamSelfAndChildren)
                    .peek(c -> c.populateFrom(
                            esQueryService.getConcept(c.getCode(), term, ip).get(), true))
        ...
        subsets.stream().flatMap(Concept::streamSelfAndChildren).peek(
                c -> c.populateFrom(esQueryService.getConcept(c.getCode(), term, ip).get(), true))
                .peek(c -> ConceptUtils.applyInclude(c, ip)).count();
