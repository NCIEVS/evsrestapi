...
public class ElasticSearchServiceImpl implements ElasticSearchService {
  ...
  public ConceptMapResultList search(String code, SearchCriteria searchCriteria) {
    ...
    final BoolQueryBuilder boolQuery = new BoolQueryBuilder().must(getMappingQuery(term, code));
  ...
  private BoolQueryBuilder getMappingQuery(String term, String code) {
    ...
    if (!term.isEmpty()) {
      ...
      if (ConceptUtils.isCode(t.replace("\\", "")) || words.size() == 1) {
        // match on either word or code, case insensitive
        termQuery.must(QueryBuilders.queryStringQuery("\"" + t + "\""));
        ...
        for (String word : words) {
          final String escapedTerm = escape(t);
          multiWordQuery.must(QueryBuilders.queryStringQuery(escapedTerm));
        }
