...
public class ElasticSearchServiceImpl implements ElasticSearchService {
  ...
  public ConceptResultList findConcepts(
    ...
    // query on operations
    final SearchHits<Concept> hits = elasticsearchOperations.search(
            searchQuery.build(),
            Concept.class,
    ...
    if (hits.getTotalHits() >= 10000) {
      result.setTotal(elasticsearchOperations.count(
              searchQuery.build(),
              Concept.class,
              IndexCoordinates.of(buildIndicesArray(searchCriteria))));
    } else {
  ...
  public ConceptMapResultList findConceptMappings(String query, SearchCriteria searchCriteria) {
    ...
    // query on operations
    final SearchHits<Mappings> hits = elasticsearchOperations.search(
            searchQuery.build(),
            Mappings.class,
