...
if [[ $skip -eq 0 ]]; then
    ...
    echo "java $local -Xmx4300M -jar $jar --terminology $terminology -d $dir --forceDeleteIndex"
    java $local -XX:+ExitOnOutOfMemoryError -Xmx4300M -jar $jar --terminology $terminology -d $dir --forceDeleteIndex
    if [[ $? -ne 0 ]]; then
