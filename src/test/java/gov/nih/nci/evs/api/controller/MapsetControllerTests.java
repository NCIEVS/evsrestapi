...
public class MapsetControllerTests {
  ...
  public void testMapsetsWithCodeForMaps() throws Exception {
    ...
    result = mvc.perform(get(baseUrl + "/GO_to_NCIt_Mapping/maps?fromRecord=1000"))
        .andExpect(status().isOk()).andReturn();
    content = result.getResponse().getContentAsString();
    mapList = new ObjectMapper().readValue(content, ConceptMapResultList.class);
    ...
    result = mvc.perform(get(baseUrl + "/SNOMEDCT_US_2020_09_01_to_ICD10CM_2021_Mappings/maps"))
        .andExpect(status().isOk()).andReturn();
    content = result.getResponse().getContentAsString();
    mapList = new ObjectMapper().readValue(content, ConceptMapResultList.class);
    assert (mapList.getMaps().size() != 0);
    ...
    result = mvc
        .perform(get(baseUrl + "/SNOMEDCT_US_2020_09_01_to_ICD10CM_2021_Mappings/maps?term=AIDS"))
        .andExpect(status().isOk()).andReturn();
    content = result.getResponse().getContentAsString();
    mapList = new ObjectMapper().readValue(content, ConceptMapResultList.class);
    assert (mapList.getMaps().size() != 0);
    ...
    result = mvc
        .perform(
            get(baseUrl + "/SNOMEDCT_US_2020_09_01_to_ICD10CM_2021_Mappings/maps?term=62479008"))
        .andExpect(status().isOk()).andReturn();
    content = result.getResponse().getContentAsString();
    mapList = new ObjectMapper().readValue(content, ConceptMapResultList.class);
    assert (mapList.getMaps().size() != 0);
    ...
    result = mvc.perform(get(baseUrl
        + "/SNOMEDCT_US_2020_09_01_to_ICD10CM_2021_Mappings/maps?sort=sourceName&ascending=true"))
        .andExpect(status().isOk()).andReturn();
    content = result.getResponse().getContentAsString();
    mapList = new ObjectMapper().readValue(content, ConceptMapResultList.class);
    assert (mapList.getMaps().size() > 1);
    assert (mapList.getMaps().get(0).getSourceName()
        .compareTo(mapList.getMaps().get(1).getSourceName()) < 0);
    ...
    mapList = new ObjectMapper().readValue(content, ConceptMapResultList.class);
    assert (mapList.getMaps().size() > 1);
    assert (mapList.getMaps().get(0).getSourceCode()
        .compareTo(mapList.getMaps().get(1).getSourceCode()) > 0);
