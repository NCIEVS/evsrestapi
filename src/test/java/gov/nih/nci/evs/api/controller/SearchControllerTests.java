...
public class SearchControllerTests {
	...
	public void testHighlight() throws Exception {
		...
		// Test a basic term search
		result = this.mvc
				.perform(get(url).param("terminology", "ncit").param("term", "melanoma").param("include", "highlights"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
	...
	public void testBadSearch() throws Exception {
		...
		mvc.perform(get(url).param("terminology", "ncit").param("term", "blood").param("pageSize", "101"))
				.andExpect(status().isBadRequest()).andReturn();
		// content is blank because of MockMvc
	...
	public void testSearchInclude() throws Exception {
		...
		// Test a basic term search
		result = this.mvc
				.perform(get(url).param("terminology", "ncit").param("term", "melanoma").param("include", "synonyms"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
	...
	public void testPageSizeFromRecord() throws Exception {
		...
		// Test a basic term search
		result = this.mvc
				.perform(get(url).param("terminology", "ncit").param("term", "melanoma").param("pageSize", "2"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		// Test a basic term search
		result = this.mvc.perform(get(url).param("terminology", "ncit").param("term", "melanoma").param("pageSize", "5")
				.param("fromRecord", "5")).andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		// Test a basic term search
		result = this.mvc
				.perform(get(url).param("terminology", "ncit").param("term", "melanoma").param("pageSize", "10"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("term", "melanoma").param("pageSize", "101"))
				.andExpect(status().isBadRequest()).andReturn();
	...
	public void testSearchProperty() throws Exception {
		...
		log.info(
				"Testing url - " + url + "?terminology=ncit&term=XAV05295I5&property=FDA_UNII_Code&include=properties");
		...
		result = this.mvc.perform(
				get(url).param("include", "properties").param("term", "XAV05295I5").param("property", "FDA_UNII_Code"))
				.andExpect(status().isOk()).andReturn();
		...
		url = baseUrl;
		log.info("Testing url - " + url + "?terminology=ncit&term=XAV05295I5&property=P319&include=properties");

		result = mvc.perform(get(url).param("terminology", "ncit").param("include", "properties")
				.param("term", "XAV05295I5").param("property", "P319")).andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		list = new ObjectMapper().readValue(content, ConceptResultList.class);
		...
		result = this.mvc
				.perform(get(url).param("include", "properties").param("term", "XAV05295I5").param("property", "P319"))
				.andExpect(status().isOk()).andReturn();
		content2 = result.getResponse().getContentAsString();
		log.info("content2 -" + content2);
	...
	public void testSearchType() throws Exception {
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("term", "malignant melanoma").param("type", "phrase"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		result = mvc
				.perform(get(url).param("terminology", "ncit").param("term", "Lung Carcinoma").param("type", "match"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("term", "lentiginous melanoma").param("type", "AND"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		result = mvc
				.perform(
						get(url).param("terminology", "ncit").param("term", "lentiginous melanoma").param("type", "OR"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
	...
	public void testConceptStatus() throws Exception {
		...
		assertThat(list.getConcepts().stream().flatMap(c -> c.getProperties().stream()).filter(
				p -> p.getType().contentEquals("Concept_Status") && p.getValue().contentEquals("Obsolete_Concept"))
		...
		// Verify property of "Obsolete_Concept on each results
		assertThat(list.getConcepts().stream().flatMap(c -> c.getProperties().stream()).filter(
				p -> p.getType().contentEquals("Concept_Status") && p.getValue().contentEquals("Provisional_Concept"))
				.count()).isEqualTo(list.getConcepts().size());
		...
		// Verify property of "Obsolete_Concept on each results
		assertThat(list.getConcepts().stream().flatMap(c -> c.getProperties().stream()).filter(
				p -> p.getType().contentEquals("Concept_Status") && p.getValue().contentEquals("Header_Concept"))
				.count()).isEqualTo(list.getConcepts().size());
		...
		result = mvc
				.perform(
						get(url).param("terminology", "ncit").param("term", "crop").param("conceptStatus", "Bad_Value"))
				.andExpect(status().isBadRequest()).andReturn();
	...
	public void testSynonymSource() throws Exception {
		...
		result = mvc
				.perform(get(url).param("terminology", "ncit").param("term", "dsDNA").param("synonymSource", "NCIT"))
				.andExpect(status().isBadRequest()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("term", "dsDNA")
				.param("synonymSource", "CDISC").param("include", "synonyms")).andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		list = new ObjectMapper().readValue(content, ConceptResultList.class);
		...
		log.info("Testing url - " + url + "?terminology=ncit&term=dsDNA&synonymSource=NCI&synonymTermGroup=SY");
		result = mvc
				.perform(get(url).param("terminology", "ncit").param("term", "dsDNA").param("synonymSource", "NCI")
		...
		for (final Synonym syn : list.getConcepts().get(0).getSynonyms()) {
			if (syn.getName().contains("dsDNA") && syn.getSource().equals("CDISC") && syn.getTermGroup().equals("SY")) {
				found = true;
				break;
			}
		}
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("term", "dsDNA").param("synonymSource", "Bad_Value"))
				.andExpect(status().isBadRequest()).andReturn();
	...
	public void testSynonymTermGroup() throws Exception {
		...
		result = mvc
				.perform(get(url).param("terminology", "ncit").param("term", "dsDNA").param("synonymTermGroup", "S"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		list = new ObjectMapper().readValue(content, ConceptResultList.class);
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("term", "dsDNA")
				.param("synonymTermGroup", "DN,SY").param("include", "synonyms")).andExpect(status().isOk())
				.andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		result = mvc
				.perform(get(url).param("terminology", "ncit").param("term", "blood").param("synonymSource", "CDISC")
						.param("pageSize", "100").param("synonymTermGroup", "SY").param("include", "summary"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		assertThat(list.getConcepts().stream()
				.filter(c -> c.getSynonyms().stream()
						.filter(s -> "SY".equals(s.getTermGroup()) && "CDISC".equals(s.getSource())).count() > 0)
				.count()).isEqualTo(list.getConcepts().size());
	...
	public void testDefinitionSource() throws Exception {
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("term", "dsDNA").param("definitionSource", "Bad_Value"))
				.andExpect(status().isBadRequest()).andReturn();

		// incomplete match
		log.info("Testing url - " + url + "?terminology=ncit&term=dsDNA&definitionSource=NC");
		result = mvc
				.perform(get(url).param("terminology", "ncit").param("term", "dsDNA").param("definitionSource", "NC"))
				.andExpect(status().isBadRequest()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content.length() == 0); // incomplete query fails
	...
	public void testSearchContains() throws Exception {
		...
		for (Concept concept : conceptList) {
			if (concept.getName().equalsIgnoreCase("braf") || !concept.getSynonyms().stream()
					.filter(p -> p.getName().equalsIgnoreCase("braf")).collect(Collectors.toList()).isEmpty()) { // found
																													// match
		...
		for (Concept concept : conceptList) {
			if (concept.getName().equalsIgnoreCase("braf")
					|| !concept.getSynonyms().stream().filter(p -> p.getName().toLowerCase().equalsIgnoreCase("braf"))
	...
	public void testSearchPhrase() throws Exception {
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("term", "malignant bone neoplasm")
				.param("pageSize", "100").param("type", "phrase").param("include", "synonyms,properties,definitions"))
				.andExpect(status().isOk()).andReturn();
	...
	public void testSearchStartsWith() throws Exception {
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("term", "malignant bone neoplasm").param("pageSize", "100")
						.param("type", "startsWith").param("include", "synonyms,properties,definitions"))
				.andExpect(status().isOk()).andReturn();
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("term", "malignant neoplasm").param("type", "startsWith")
		...
		for (Concept concept : conceptList) {
			assertTrue((concept.getName().toLowerCase().contains("malignant")
					...
					|| !concept.getSynonyms().stream().filter(p -> p.getName().toLowerCase().contains("malignant"))
							...
							.filter(p -> p.getName().toLowerCase().contains("neoplasm")).collect(Collectors.toList())
							.isEmpty()
					|| !concept.getProperties().stream().filter(p -> p.getValue().toLowerCase().contains("malignant"))
							...
							.filter(p -> p.getValue().toLowerCase().contains("neoplasm")).collect(Collectors.toList())
							.isEmpty()
					|| !concept.getDefinitions().stream()
	...
	public void testSearchOR() throws Exception {
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("term", "malignant bone neoplasm")
				.param("pageSize", "100").param("type", "OR").param("include", "synonyms,properties,definitions"))
				.andExpect(status().isOk()).andReturn();
	...
	public void testSearchExact() throws Exception {
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("term", "malignant bone neoplasm")
				.param("pageSize", "100").param("type", "match").param("include", "synonyms,properties,definitions"))
				.andExpect(status().isOk()).andReturn();
	...
	public void testSearchByConceptCode() throws Exception {
		...
		// Test startsWith
		result = this.mvc
				.perform(get(url).param("terminology", "ncit").param("term", "C3224").param("type", "startsWith"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
		...
		// Test contains
		result = this.mvc
				.perform(get(url).param("terminology", "ncit").param("term", "C3224").param("type", "contains"))
				.andExpect(status().isOk()).andReturn();
		content = result.getResponse().getContentAsString();
		log.info("  content = " + content);
		assertThat(content).isNotNull();
	...
	public void testSearchBlank() throws Exception {
		...
		result = mvc.perform(
				get(url).param("terminology", "ncit").param("synonymSource", "GDC").param("include", "synonyms"))
				.andExpect(status().isOk()).andReturn();
		list = new ObjectMapper().readValue(result.getResponse().getContentAsString(), ConceptResultList.class);
		assertThat(list.getConcepts() != null && list.getConcepts().size() > 0).isTrue();
		...
		result = mvc.perform(get(url).param("terminology", "ncit").param("definitionSource", "CDISC").param("include",
				"definitions")).andExpect(status().isOk()).andReturn();
		list = new ObjectMapper().readValue(result.getResponse().getContentAsString(), ConceptResultList.class);
		assertThat(list.getConcepts() != null && list.getConcepts().size() > 0).isTrue();
		for (final Concept conc : list.getConcepts()) { // test that have match
														// to
	...
	public void testBadSearchTerm() throws Exception {
		...
		for (final String type : new String[] { "contains", "fuzzy", "phrase", "match", "startsWith", "AND", "OR" }) {
		...
		for (final String type : new String[] { "contains", "fuzzy", "phrase", "match", "startsWith", "AND", "OR" }) {
			...
			result = mvc.perform(
					get(url).param("terminology", "ncit").param("term", "C)%26ghd melanoma").param("type", type))
					.andExpect(status().isOk()).andReturn();
			String content = result.getResponse().getContentAsString();
			log.info("  content = " + content);
			assertThat(content).isNotNull();
