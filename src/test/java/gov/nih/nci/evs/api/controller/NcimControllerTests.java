...
public class NcimControllerTests {
  ...
  public void testMRSTY() throws Exception {
                ...
                .filter(
                    p ->
                        p.getType().equals("Semantic_Type")
                            && p.getValue().equals("Amino Acid, Peptide, or Protein"))
                .count())
        .isEqualTo(1);
    ...
    assertThat(concept.getName())
        .isEqualTo(
            "Guidance for drainage+placement of drainage catheter^WO"
                + " contrast:Find:Pt:Abdomen:Doc:CT");
  ...
  public void testMRREL2() throws Exception {
    ...
    assertThat(
        concept.getParents().stream()
                .filter(
                    r ->
                        !r.getQualifiers().isEmpty()
                            && r.getQualifiers().get(0).getValue().equals("isa"))
                .count()
            > 0);
    ...
    assertThat(
        concept.getChildren().stream()
                .filter(
                    r ->
                        !r.getQualifiers().isEmpty()
                            && r.getQualifiers().get(0).getValue().equals("inverse_isa"))
                .count()
            > 0);
    ...
    assertThat(
        concept.getAssociations().stream()
                .filter(
                    r ->
                        !r.getQualifiers().isEmpty()
                            && r.getQualifiers().get(0).getValue().equals("analyzed_by"))
                .count()
            > 0);
    assertThat(
        concept.getInverseAssociations().stream()
                .filter(
                    r ->
                        !r.getQualifiers().isEmpty()
                            && r.getQualifiers().get(0).getValue().equals("analyzes"))
                .count()
            > 0);
  ...
  public void testMRSAT() throws Exception {
    ...
    assertThat(
            concept.getProperties().stream()
                .filter(
                    p ->
                        p.getType().equals("DEFINITION_STATUS_ID")
                            && p.getSource().equals("SNOMEDCT_US")
                            && p.getValue().equals("900000000000074008"))
                .count())
        .isEqualTo(1);
  ...
  public void testNcimSearchContains() throws Exception {
    ...
    assertThat(
            list.getConcepts().stream()
                .filter(c -> c.getCode().equals("C0004057"))
