...
public class GOControllerTests {
  ...
  /** The logger. */
  private static final Logger log = LoggerFactory.getLogger(GOControllerTests.class);
  ...
  //
  // /**
  // * MRDEF basic tests. N/A - no GO definitions
  // *
  // * @throws Exception the exception
  // */
  // // @Test
  ...
  // assertThat(concept.getProperties().stream().filter(p ->
  // p.getType().equals("Semantic_Type"))
  ...
  // assertThat(concept.getAssociations().stream().filter(a ->
  // a.getRelatedCode().equals("10013206"))
  // .findFirst().orElse(null)).isNotNull();
  // assertThat(concept.getAssociations().stream().filter(a ->
  // a.getRelatedCode().equals("10013206"))
  ...
  // .filter(a ->
  // a.getRelatedCode().equals("10064732")).findFirst().orElse(null)).isNotNull();
  // assertThat(
  // concept.getInverseAssociations().stream().filter(a ->
  // a.getRelatedCode().equals("10064732"))
  ...
  // assertThat(concept.getChildren().stream().map(c ->
  ...
  // assertThat(concept.getParents().stream().map(c ->
  // c.getCode()).collect(Collectors.toSet()))
  // .contains("10053567");
  //
  // }
  ...
  // .filter(p -> p.getType().equals("PRIMARY_SOC") &&
  // p.getValue().equals("10005329")).count())
  // .isEqualTo(1);
  //
  ...
  // assertThat(concept.getAssociations().stream().filter(a ->
  // a.getQualifiers().size() > 1).count())
  // .isEqualTo(1);
  // assertThat(concept.getAssociations().stream().filter(a ->
  // a.getQualifiers().size() > 1)
  // .flatMap(a -> a.getQualifiers().stream()).filter(q ->
  // q.getType().equals("SMQ_TERM_CAT"))
  // .count()).isEqualTo(1);
  //
  ...
  // a.getQualifiers().size() > 1).count())
  // .isEqualTo(1);
  // assertThat(concept.getInverseAssociations().stream().filter(a ->
  // a.getQualifiers().size() > 1)
  // .flatMap(a -> a.getQualifiers().stream()).filter(q ->
  // q.getType().equals("SMQ_TERM_CAT"))
  // .count()).isEqualTo(1);
  ...
  // .param("pageSize", "100").param("type", "contains").param("include",
  // "synonyms"))
  ...
  // assertThat(list.getConcepts().stream().filter(c ->
  // c.getCode().equals("10009729"))
  // .collect(Collectors.toList()).size()).isEqualTo(1);
  //
  // // Test searching "Hepatitis, non-infectious (SMQ)"
  // log.info("Testing url - " + url
  // +
  // "?terminology=mdr&fromRecord=0&include=synonyms&pageSize=100&term=Hepatitis,
  // non-infectious (SMQ)&type=contains");
  // result = mvc
  // .perform(get(url).param("terminology", "mdr").param("term", "Hepatitis,
  // non-infectious (SMQ)")
  // .param("pageSize", "100").param("type", "contains").param("include",
  // "synonyms"))
  // .andExpect(status().isOk()).andReturn();
  ...
  // .perform(get(url).param("terminology", "mdr").param("term", "Hepatitis,
  // non-infectious (SMQ)")
  // .param("pageSize", "100").param("type", "match").param("include",
  // "synonyms"))
  // .andExpect(status().isOk()).andReturn();
  // // Just checking that searching with parentheses doesn't cause an error
  // }
  ...
  // assertThat(list.stream().map(c ->
  // c.getCode()).collect(Collectors.toSet())).contains("RELA");
  //
  ...
  // assertThat(list.stream().map(c ->
  // c.getCode()).collect(Collectors.toSet())).contains("MDR");
  //
  // // Handle synonymTypes
  ...
  // assertThat(list.stream().map(c ->
  // c.getCode()).collect(Collectors.toSet())).contains("LLT");
  // assertThat(list.stream().map(c ->
  // c.getCode()).collect(Collectors.toSet())).contains("PT");
  //
  // }
  //
  // /**
  ...
  // content = result.getResponse().getContentAsString();
  // log.info(" content = " + content);
  // list2 = new ObjectMapper().readValue(content, new
  // TypeReference<List<HierarchyNode>>() {
  // // n/a
  // });
