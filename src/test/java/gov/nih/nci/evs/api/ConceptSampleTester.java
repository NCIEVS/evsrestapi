...
public class ConceptSampleTester {
  ...
  public boolean checkQualifier(Concept concept, SampleRecord sample) throws Exception {
    String qualKey = sample.getKey().split("-")[1].split("~")[0];
    String qualValue = sample.getValue().split("-")[1].split("~")[0];
    String propertyKey = sample.getKey().split("-")[1].split("~")[1];
    String propertyValue = sample.getValue().split("-")[1].split("~")[1];
    if (terminology.getMetadata().getSynonym().contains(propertyKey)) {
      ...
      ;
    } else if (terminology.getMetadata().getMap().equals(propertyKey)) {
      ;
    } else {
      ;
    }
